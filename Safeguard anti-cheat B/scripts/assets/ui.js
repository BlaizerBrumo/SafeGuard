...
import { invsee, unban, millisecondTime, sendMessageToAllAdmins } from './util.js';
	...
	if(type == "quick"){
		...
		confirmF.show(player).then((confirmData) => {
			if(confirmData.selection === 1){
				...
				const now = Date.now();
				targetPlayer.addTag(`safeguardBanInfo**true**${now}**${player.name}**No reason provided.`);
				...
				sendMessageToAllAdmins(`"§6[§eSafeGuard Notify§6]§5§l ${player.name} §bbanned§l§5 ${targetPlayer.name}! §r`,true);
...
function moduleSettingsForm(player){	
	const settings = ["anti_gmc","anti_grief","anti_items","auto_mod","death_coords","death_effect","lock_end","welcomer","anti_autoclicker","anti_killaura","anti_nuker","anti_spammer"];
	const settingsName = ["Anti GMC","Anti Grief","Anti Items","Auto Mod","Death Coords","Death Effect","Lock End","Welcomer","High CPS Check","Anti Killaura","Anti Block Nuker","Anti Spammer"];
	const settingsScoreboard = ["gmc_on","grief_on","item_on","auto_mod_on","death_coord_on","death_effect","end_lock","welcome_on","safeguard:cps_check","safeguard:killaura_check","safeguard:nuker_check","safeguard:spammer_protection"];
...
function betaFeatureSettingsForm(player){
	
	const settings = ["anti_fly","anti_scaffold","invalid_velocity_check","combat_logging"];
	const settingsName = ["Anti Fly","Anti Scaffold","Invalid Velocity Check","Anti Combat Logging"];
	const settingsScoreboard = ["safeguard:fly_check","safeguard:scaffold_check","safeguard:velocity_check","safeguard:anti_combatlog"];

	let betaFeaturesForm = new ModalFormData()
	.title("SafeGuard Beta Feature Settings")
	...
	for (let i = 0; i < settings.length; i++) {
		...
		betaFeaturesForm.toggle(setting, isSettingEnabled);
	}

	betaFeaturesForm.show(player).then((formData) => {
		if (formData.canceled) {
			player.sendMessage(`§6[§eSafeGuard§6]§r You closed the form without saving!`);
			return;
		}
		...
		for (let i = 0; i < settings.length; i++) {
			...
			if(shouldEnableSetting && !warningDisplayed){
				warningDisplayed = true;
				player.sendMessage(`§6[§eSafeGuard§6] §4Warning!§c You just turned on a beta feature, these features are still being tested and could potentially have false positive, for this, SafeGuard sends alerts for beta features only to admins. Submit your feedback on these features in the offical SafeGuard discord: discord.gg/nGu5gehXs3§r`)
			}

			if (isSettingEnabled !== shouldEnableSetting) {
				player.runCommand(`function settings/beta/${setting}`);
			}
		}
	})
} 
...
export function settingsForm(player){
	...
	form.show(player).then((formData) => {
		...
		if(formData.selection === 2) return betaFeatureSettingsForm(player);
	})
}
	...
	form.show(player).then((formData) => {
		...
		switch(action){
			...
			case 5:
				...
				break;
			case 6:
				return invsee(player.name,targetPlayer.name);
				break
			case 7:
