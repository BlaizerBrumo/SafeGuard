...
newCommand({
    ...
    run: (data) => {
        ...
        if (args[1].startsWith('"') || args[1].startsWith('@"')) {
            ...
            const playerName = args.slice(1, closingQuoteIndex + 1).join(" ").replace(/["@]/g, "");
            const reason = args.slice(closingQuoteIndex + 1).join(" ") ?? "No reason provided";
            const reportedPlayer = getPlayerByName(playerName);

            if (!reportedPlayer) return player.sendMessage(`§6[§eSafeGuard§6]§f Player §e${playerName}§f was not found`);
            
            

            reportPlayer(reportedPlayer,reason);
        } else {
            const playerName = args[1].replace(/["@]/g, "");
            const reportedPlayer = getPlayerByName(playerName);
            if (!reportedPlayer) return player.sendMessage(`§6[§eSafeGuard§6]§f Player §e${playerName}§f was not found`);
        ...
        function reportPlayer(reportedPlayer,reason){
            if (reportedPlayer.name === player.name) return player.sendMessage(`§6[§eSafeGuard§6]§f Cannot execute this command on yourself!`);
